   ...
   DSL_PM_LineInitData_t *pCounters)
{
   ...
   DSL_DEBUG( DSL_DBG_MSG,
      (pContext, SYS_DBG_MSG"DSL[%02d]: OUT - DSL_DRV_PM_DEV_LineInitCountersSet, retCode: %d"
      DSL_DRV_CRLF, DSL_DEV_NUM(pContext), nErrCode));
   ...
   DSL_PM_ReTxData_t *pCounters)
{
   ...
   if (nVerCheck >= DSL_VERSION_EQUAL)
   {
      ...
      /* EFTR has different handling for NE and FE   */
      /* FE  =>  CMD/ACK_RTX_PM_US_Get::EFTR_min_xxx */
      /* NE  =>  CMD/ACK_RTX_PM_DS_Get::EFTR_xxx     */
      if (nDirection == DSL_FAR_END)
      {
         pCounters->nErrorFreeBits = ((DSL_uint32_t)(retxPm.FUS.ErrorFreeBits_MSW << 16)) |
                                                     retxPm.FUS.ErrorFreeBits_LSW;
         pCounters->nLeftr         = ((DSL_uint32_t)(retxPm.FUS.leftr_MSW << 16)) |
                                                     retxPm.FUS.leftr_LSW;
         nEftrMin                  = ((DSL_uint32_t)(retxPm.FUS.EFTR_min_MSW << 16)) |
                                                     retxPm.FUS.EFTR_min_LSW;
      }
      else
      {
         pCounters->nErrorFreeBits = ((DSL_uint32_t)(retxPm.FDS.ErrorFreeBits_MSW << 16)) |
                                                     retxPm.FDS.ErrorFreeBits_LSW;
         pCounters->nLeftr         = ((DSL_uint32_t)(retxPm.FDS.leftr_MSW << 16)) |
                                                     retxPm.FDS.leftr_LSW;
         nEftrMin                  = ((DSL_uint32_t)(retxPm.FDS.EFTR_MSW << 16)) |
                                                     retxPm.FDS.EFTR_LSW;
      }
...
DSL_Error_t DSL_DRV_PM_DEV_ShowtimeCheckHandler(
   DSL_Context_t *pContext,
   DSL_boolean_t *bMsgSendRequired)
{
   DSL_Error_t nErrCode = DSL_SUCCESS;
   ...
   if((nLineState == DSL_LINESTATE_SHOWTIME_NO_SYNC) ||
      ...
      /* save PM counters at fail state */
      (nLineState == DSL_LINESTATE_EXCEPTION))
   {
      *bMsgSendRequired = DSL_TRUE;
   }
   else
   {
      *bMsgSendRequired = DSL_FALSE;
   }
