   ...
   DSL_Context_t *pContext)
{
   ...
   DSL_CTX_READ(pContext, nErrCode, BndConfig[nDslMode], BndConfig);
   ...
   if (BndConfig.bPafEnable == DSL_TRUE)
   {
      ...
      {
         /* VDSL2 - Get FW information*/
         nErrCode = DSL_DRV_VRX_FirmwareVersionCheck(pContext,
                     DSL_MIN_FW_VERSION_R9_RELEASE, &nVerCheck);
      }
      else
      {
         /* ADSL - Get FW information*/
         nErrCode = DSL_DRV_VRX_FirmwareVersionCheck(pContext,
                     DSL_MIN_FW_VERSION_ADSL_PTM_BONDING, &nVerCheck);
      }
      ...
      {
         nErrCode = DSL_DRV_VRX_SendMsgPmeIdSet(pContext,
                                    (const DSL_uint16_t)(DSL_DEV_NUM(pContext)));
         if (nErrCode < DSL_SUCCESS)
         {
            DSL_DEBUG( DSL_DBG_ERR,
               (pContext, SYS_DBG_ERR"DSL[%02d]: ERROR - PME ID set failed!"
               DSL_DRV_CRLF, DSL_DEV_NUM(pContext)));
