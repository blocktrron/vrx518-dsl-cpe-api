...
DSL_void_t DSL_DRV_Cleanup(DSL_void_t)
{
   ...
   for (i = 0; i < DSL_DRV_ENTITIES; i++)
   {
      ...
      pr_info("Reset and Halt DFE %d"DSL_DRV_CRLF, i);
      DSL_BSP_KernelIoctls(ifxDevices[i].lowHandle, DSL_FIO_BSP_RESET, (unsigned long)DSL_NULL);
      DSL_BSP_KernelIoctls(ifxDevices[i].lowHandle, DSL_FIO_BSP_HALT, (unsigned long)DSL_NULL);
#endif
